...
export const getAnimationsForSlug = (slug = "") => {
  ...
  if (s.includes("usa") || s.includes("july4") || s.includes("4th") || s.includes("independence") || s.includes("july")) return "july4";
  ...
  if (s.includes("condolence") || s.includes("rip") || s.includes("loss") || s.includes("funeral")) return "condolences";
  return "general";
};
...
const OPTIONS = {
  ...
  condolences: [
    "✨ None (No Animation)",
    "Candles 🕯️","Hearts 🤍","Doves 🕊️","Stars ⭐","Clouds ☁️",
    "Flowers 🌸","Ribbons 🎀","Peace ☮️","Wings 🪽","Light 💫"
  ],
  general: [
    "✨ None (No Animation)",
...
export function AnimationOverlay({
  ...
  // extrae el emoji del label seleccionado (p.ej. "Snow ❄️" -> "❄️")
  const emoji = animation.match(/[\p{Emoji}\u200d]+/gu)?.[0] || "✨";

  // estado seguro para SSR: viewport
  const [viewportWidth, setViewportWidth] = useState(1024);
  useEffect(() => {
    const update = () => setViewportWidth(typeof window !== "undefined" ? window.innerWidth || 1024 : 1024);
    update();
  ...
  const config = {
    ...
    normal: { mult: 1.0, dur: 8,  op: opacityLevel },
    vivid:  { mult: 1.5, dur: 6,  op: Math.min(1, opacityLevel * 1.1) },
  };
  const level = config[intensity] || config.normal;

  useEffect(() => {
    // base por tamaño de pantalla
    const base =
      viewportWidth < 480  ? 10 :
      viewportWidth < 768  ? 14 :
      viewportWidth < 1200 ? 18 : 24;

    // cantidad final: base * intensidad * control usuario (emojiCount/20)
    const count = Math.max(1, Math.round(base * level.mult * (emojiCount / 20)));
    ...
    setItems(arr);
  }, [slug, animation, intensity, emojiCount, opacityLevel, viewportWidth]);

  // tamaño de fuente según pantalla
  const baseSize =
    viewportWidth < 480  ? 14 :
    viewportWidth < 768  ? 16 :
    viewportWidth < 1200 ? 20 : 24;

  return (
    <div className="pointer-events-none fixed inset-0 overflow-hidden" style={{ zIndex: 5000 }}>
      {items.map((it) => (
        <motion.span
          key={it.id}
          ...
          style={{
            ...
            opacity: level.op,
            // 💡 mejora visual: resalta emojis grisáceos para que no se "pierdan"
            filter: emoji.match(/❄️|☁️|💔|🕯️|⚪|🧊|🌫️|⚙️/)
              ? "brightness(1.7) contrast(1.3)"
              : "none",
          }}
