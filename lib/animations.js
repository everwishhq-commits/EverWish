...
/* =========================================================
   💌 MENSAJES AUTOMÁTICOS (3 por categoría)
   ========================================================= */
...
export function parseCategoryFromSlug(slug = "") {
  ...
  if (/(condolence|sympathy|loss|funeral|memory)/.test(s)) return "condolence";
  if (/(baby|shower)/.test(s)) return "babyshower";
  if (/(independence|july|usa|freedom)/.test(s)) return "independence";
  return "general";
}
