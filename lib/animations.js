...
/* -----------------------------------
 *  ASIGNACIÓN DE CATEGORÍAS POR SLUG
 * ----------------------------------- */
const slugToCategory = (slug = "", message = "") => {
  ...
  if (s.includes("halloween") || s.includes("ghost") || s.includes("zombie")) return "halloween";
...
const OPTIONS = {
  ...
  halloween: ["Pumpkins 🎃","Ghosts 👻","Candy 🍬","Bats 🦇","Spiders 🕷️","Moon 🌙","Stars ✨","Skulls 💀","Fog 🌫️","Sparkles ✨"],
  birthday:  ["Confetti 🎊","Balloons 🎈","Cake 🎂","Candles 🕯️","Stars ⭐","Gifts 🎁","Streamers 🎉","Hearts ❤️","Sparkles ✨","Party 🥳"],
  easter:    ["Eggs 🥚","Bunnies 🐰","Flowers 🌸","Carrots 🥕","Butterflies 🦋","Sun ☀️","Grass 🌿","Rain ☔","Sparkles ✨","Spring 🌼"],
...
export function AnimationOverlay({ slug, message, animation }) {
  ...
  useEffect(() => {
    ...
    setItems(next);
  }, [emojis, animation]); // 🔥 Actualización clave para refrescar
