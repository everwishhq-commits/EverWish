...
export default function CheckoutModal({
  ...
  /* 🔹 Actualiza descripción, precio y preview dinámicamente */
  useEffect(() => {
    if (plan === "animated") {
      setPrice(3.99);
      setDescription(
        "🎬 Animated MP4 Card — Bring your wish to life with movement and sound."
      );
  ...
  const handlePurchase = () => {
    ...
    setTimeout(() => {
      ...
      alert(
        `✅ Purchase successful — ${
          plan === "animated" ? "Animated MP4 Card" : "Heartfelt Photo Card"
        } $${price.toFixed(2)}`
      );
      onClose();
    }, 1500);
