...
export default function CropperModal({ open, onClose, onDone }) {
  ...
  const draw = (ctx, img, scale, offset) => {
    ...
    // 🔧 Fondo blanco para eliminar los bordes negros
    ctx.fillStyle = "#fff";
    ctx.fillRect(0, 0, width, height);

    // Mantener proporción al ancho y llenar completamente el área visible
  ...
  const handleDone = () => {
    const canvas = canvasRef.current;
    // ✨ Exporta la imagen sin los bordes negros
    ...
    >
      <div className="bg-white rounded-3xl shadow-xl p-4 w-[94%] max-w-sm flex flex-col gap-3">
        {!imageSrc ? (
          ...
          <>
            {/* 🔧 Canvas ajustado sin bordes negros */}
            <canvas
              ref={canvasRef}
              width={360}
              height={270} // tamaño equilibrado y adaptable
              className="rounded-xl bg-white w-full h-auto touch-none object-contain"
