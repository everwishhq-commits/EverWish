...
export default function CropperModal({ open, onClose, onDone }) {
  ...
  // Evita el refresh por scroll o gesto "pull down" en móvil
  useEffect(() => {
  ...
  const draw = (ctx, img, scale, offset) => {
    ...
    // Calcula tamaño ajustado al contenedor
    const aspectRatio = img.width / img.height;
    let displayWidth = width * scale;
    let displayHeight = displayWidth / aspectRatio;

    if (displayHeight < height * 0.6) {
      displayHeight = height * 0.6;
      displayWidth = displayHeight * aspectRatio;
    }

    const x = width / 2 - displayWidth / 2 + offset.x;
    const y = height / 2 - displayHeight / 2 + offset.y;

    ctx.drawImage(img, x, y, displayWidth, displayHeight);

    // Cuadrícula (9 cuadros)
    const cropX = width / 10;
    const cropY = height / 10;
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl shadow-xl p-4 w-[92%] max-w-sm flex flex-col gap-3 select-none">
        {!imageSrc ? (
          <input
            type="file"
            accept="image/*"
