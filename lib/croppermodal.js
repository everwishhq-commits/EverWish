  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-[90%] max-w-md p-5 flex flex-col items-center relative">
        {/* Si no hay imagen, mostrar input */}
        {!imageSrc ? (
          ...
          <>
            {/* Vista táctil de recorte */}
            <div className="relative w-full h-64 bg-gray-100 rounded-xl overflow-hidden border-2 border-pink-200 shadow-inner touch-none">
              <Cropper
                image={imageSrc}
                crop={crop}
                zoom={zoom}
                ...
                minZoom={1}
                maxZoom={3}
                zoomWithTouch={true} // 👈 permite zoom con dedos
              />
            </div>
