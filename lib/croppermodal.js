...
export default function CropperModal({ open, onClose, onDone }) {
  ...
  // 🔒 Bloquea el scroll y el "pull-to-refresh" mientras el modal está abierto
  useEffect(() => {
    if (open) {
  ...
  const draw = (ctx, img, scale, offset) => {
    ...
    // Cuadrado de recorte
    const cropX = width / 4;
    const cropY = height / 4;
    const cropSize = width / 2;
  ...
  const handleTouchMove = (e) => {
    e.preventDefault(); // 🧱 Bloquea refresh o scroll del navegador
