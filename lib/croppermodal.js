...
export default function CropperModal({ open, onClose, onDone }) {
  ...
  const draw = (ctx, img, scale, offset) => {
    ...
    // fondo gris suave para distinguir límites
    ctx.fillStyle = "#222";
  ...
  const handleTouchMove = (e) => {
    ...
    if (e.touches.length === 1) {
      ...
      lastTouch.current = { x: e.touches[0].clientX, y: e.touches[0].clientY };
      e.preventDefault(); // Evita el pull-to-refresh
    }
  };
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl shadow-xl p-4 w-[94%] max-w-sm flex flex-col gap-3 overflow-hidden">
        {!imageSrc ? (
          <input
            type="file"
            accept="image/*"
