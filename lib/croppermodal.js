...
export default function CropperModal({ open, onClose, onDone, existingImage, onDelete }) {
  ...
  // Convierte el recorte a base64 para guardar
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-[92%] max-w-md p-4 flex flex-col items-center relative">
        {/* Subir imagen si no hay */}
        {!imageSrc ? (
          <div className="flex flex-col items-center justify-center w-full h-52">
            <p className="text-gray-700 font-semibold mb-3">Upload your image</p>
            <input
              type="file"
              accept="image/*"
          ...
          <>
            {/* Área de recorte táctil con cuadrícula */}
            <div className="relative w-full h-64 bg-gray-100 rounded-xl overflow-hidden border-2 border-pink-200 shadow-inner">
              <Cropper
                image={imageSrc}
                crop={crop}
                zoom={zoom}
                ...
                zoomWithTouch={true} // 👈 permite zoom con los dedos
