...
function parseCategories(slug) {
  ...
  // si termina en "general", ignoramos subcategorías redundantes (ya vienen arriba)
...
function getAnimationsForSlug(slug) {
  ...
  // combina categorías (máx 10 únicas)
  const bag = [];
...
export default function EditPage() {
  ...
  const [anim, setAnim] = useState(""); // texto como "🎃 Pumpkin Glow"
  const CARD_PRICE = 5;
  ...
  /* ---------- Fullscreen + barra progreso suave ---------- */
  ...
  /* ---------- Animación render (suave) ---------- */
  const renderEffect = () => {
    if (!anim || /None/.test(anim)) return null;
    const emoji = anim.split(" ")[0]; // primer token emoji
    return Array.from({ length: 18 }).map((_, i) => (
      <motion.span
        ...
        transition={{
          duration: 4.5 + Math.random() * 2, // más suave + lento
          repeat: Infinity,
          ease: "easeInOut",
          delay: i * 0.22,
        }}
  ...
  /* ---------- Intro pantalla completa (tap para iniciar) ---------- */
  if (!item) return null;
    ...
    <main className="mx-auto max-w-3xl px-4 py-8 relative bg-[#fff8f5] min-h-screen overflow-hidden">
      ...
      <div className="relative z-20">
        ...
        <section className="mt-6 bg-white rounded-3xl shadow-md p-6">
          ...
          {/* Dropdown de animaciones (10 según categorías detectadas) */}
