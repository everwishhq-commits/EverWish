...
function CheckoutForm({ total, gift, onSuccess, onError, isAdmin, cardData }) {
  ...
  const handleAdminSend = async (e) => {
    ...
    try {
      ...
      onSuccess({ type: "admin_send", data });
    } catch (err) {
  ...
  return (
    <form onSubmit={isAdmin ? handleAdminSend : handlePayment} className="space-y-4">
      ...
      {isAdmin && (
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 text-purple-700 text-sm flex items-center gap-2">
      ...
      <button
        ...
        className={`w-full rounded-xl py-4 font-bold text-white text-lg flex items-center justify-center gap-2 ${
          processing
